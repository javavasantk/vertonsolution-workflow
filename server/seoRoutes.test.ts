...
describe("explicit crawler routes", () => {
  it("serves robots and sitemap before SPA fallback middleware", async () => {
    ...
    expect(await robots.text()).toContain("Sitemap: https://novarocketrylearning.best/sitemap.xml");
    const sitemap = await get("/sitemap.xml");
    expect(sitemap.headers.get("content-type")).toContain("application/xml");
    expect(await sitemap.text()).toContain("https://novarocketrylearning.best/aeroforge");
  });
  ...
  it("serves the manifest and security policy with bot-friendly content types", async () => {
    ...
    expect(await security.text()).toContain("Contact: https://novarocketrylearning.best/contact");
  });
});
