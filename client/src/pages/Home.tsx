...
function Workspace({ exitWorkspace, requestedPage = "Overview" }: { exitWorkspace: () => void; requestedPage?: string }) {
  ...
  const workspaceAssistantMutation = trpc.ai.workspaceAssistant.useMutation({
    onSuccess: data => setAssistantMessages(messages => [...messages, { role: "assistant", content: data.reply }]),
  });
  ...
  const assistantPrompts = activeRole === "Recruiter"
    ? ["What should I review before a candidate handoff?", "How do I use Candidate Finder?"]
  ...
  const PageContent = () => {
    ...
    const content = activePage === "Talent pipeline" ? <Talent />
      ...
      : activePage === "Delivery" ? <><Delivery /><DatabaseDeliveryLifecycle /></>
      ...
      : activePage === "New-hire progress" ? <><RecruiterDashboard />{ResumeParserPanel()}</>
      : <Overview />;
