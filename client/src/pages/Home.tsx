...
function Workspace({ exitWorkspace, requestedPage = "Overview" }: { exitWorkspace: () => void; requestedPage?: string }) {
  ...
  const previewRole = typeof window !== "undefined" && import.meta.env.DEV
    ...
    : null;
  const previewMode = previewRole !== null && ["admin", "recruiter", "hr_compliance", "account_manager", "delivery_manager", "project_manager", "finance", "consultant"].includes(previewRole);
  const activeRole = getRoleKeyFromStoredRole(previewMode ? previewRole : user?.role);
