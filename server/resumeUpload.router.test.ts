...
const { validateResumeMetadataSpy, extractResumeTextFromBytesSpy, parseRecruiterResumeSpy, storageCreateUploadTargetSpy, storageGetSignedUrlSpy, createResumeUploadSessionSpy, getActiveResumeUploadSessionSpy, completeResumeUploadSessionSpy, createCandidateProfileSpy } = vi.hoisted(() => ({ validateResumeMetadataSpy: vi.fn(), extractResumeTextFromBytesSpy: vi.fn(), parseRecruiterResumeSpy: vi.fn(), storageCreateUploadTargetSpy: vi.fn(), storageGetSignedUrlSpy: vi.fn(), createResumeUploadSessionSpy: vi.fn(), getActiveResumeUploadSessionSpy: vi.fn(), completeResumeUploadSessionSpy: vi.fn(), createCandidateProfileSpy: vi.fn() }));
...
vi.mock("./storage", () => ({ storageCreateUploadTarget: storageCreateUploadTargetSpy, storageGetSignedUrl: storageGetSignedUrlSpy }));
...
describe("recruiting signed resume upload", () => {
  beforeEach(() => {
    ...
    validateResumeMetadataSpy.mockReset(); extractResumeTextFromBytesSpy.mockReset(); parseRecruiterResumeSpy.mockReset(); storageCreateUploadTargetSpy.mockReset(); storageGetSignedUrlSpy.mockReset(); createResumeUploadSessionSpy.mockReset(); getActiveResumeUploadSessionSpy.mockReset(); completeResumeUploadSessionSpy.mockReset(); createCandidateProfileSpy.mockReset();
    ...
    storageCreateUploadTargetSpy.mockResolvedValue({ key: session.fileKey, uploadUrl: "https://upload.example.test/resume" });
  ...
  it("issues a recruiter-bound short-lived target then completes parsing from stored bytes", async () => {
    ...
    expect(prepared).toMatchObject({ sessionId: session.id, uploadUrl: "https://upload.example.test/resume" });
    expect(storageCreateUploadTargetSpy).toHaveBeenCalledWith(session.fileKey, "application/pdf");
  ...
  it("rejects non-recruiters and invalid upload metadata before a storage target is created", async () => {
    ...
    expect(storageCreateUploadTargetSpy).not.toHaveBeenCalled();
  });
