...
describe("auth.signIn", () => {
  ...
  it("accepts correct credentials and writes the JWT session cookie", async () => {
    getUserByEmail.mockResolvedValue({ id: 23, openId: "pw-auth-test", name: "Aero Student", email: "student@example.invalid", passwordHash: await hashPassword("strong-passphrase"), planId: "explorer", sessionVersion: 0, role: "user", avatarUrl: null, loginMethod: "password", createdAt: new Date(), updatedAt: new Date(), lastSignedIn: new Date() });
    ...
    expect(createSessionToken).toHaveBeenCalledWith("pw-auth-test", { name: "Aero Student", sessionVersion: 0 });
