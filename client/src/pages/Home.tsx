...
function CredentialLogin({ openWorkspace }: { openWorkspace: () => void }) {
  ...
  return <div className="min-h-screen bg-[#f7faff] text-[#12345a]"><div className="grid min-h-screen lg:grid-cols-[.95fr_1.05fr]"><section className="relative hidden overflow-hidden bg-[#09264b] px-10 py-12 text-white lg:flex lg:flex-col lg:justify-between"><div className="verton-grid absolute inset-0 opacity-60" /><div className="relative"><Logo dark /><p className="mt-20 font-mono-ui text-[10px] font-bold uppercase tracking-[.18em] text-[#78e2d4]">Secure workforce operations</p><h1 className="mt-4 max-w-xl text-5xl font-extrabold tracking-[-.065em]">One secure entry point for your assigned work.</h1><p className="mt-6 max-w-lg text-sm leading-7 text-blue-100/75">Workforce Hub protects sensitive operations with account-based access and role-scoped workspaces.</p></div><div className="relative rounded-2xl border border-white/10 bg-white/6 p-4"><ShieldCheck className="text-[#78e2d4]" size={18} /><p className="mt-3 text-xs font-bold">Controlled access</p><p className="mt-1 text-[10px] leading-4 text-blue-100/65">Your workspace and available actions are determined after sign-in.</p></div></section><section className="flex items-center justify-center px-5 py-10 sm:px-8"><div className="w-full max-w-md"><div className="lg:hidden"><Logo /></div><div className="mt-10 rounded-[24px] border border-[#dce7f3] bg-white p-6 shadow-[0_22px_50px_rgba(18,52,90,.10)] sm:mt-0 sm:p-8"><div className="grid h-10 w-10 place-items-center rounded-xl bg-[#e8f2ff] text-[#0b57d0]">{mode === "login" ? <LockKeyhole size={19} /> : <Mail size={19} />}</div><p className="mt-6 font-mono-ui text-[10px] font-bold uppercase tracking-[.15em] text-[#0b57d0]">First login</p><h2 className="mt-2 text-3xl font-extrabold tracking-[-.055em]">{heading}</h2><p className="mt-3 text-sm leading-6 text-[#7185a0]">{description}</p>{mode === "login" && <><button onClick={() => startLogin()} className="mt-5 rounded-xl border border-[#d6e3f1] bg-white px-3.5 py-2.5 text-xs font-bold text-[#315678] shadow-sm transition hover:border-[#8fb7ee]">Use approved identity <ArrowRight className="ml-1 inline" size={14} /></button><form onSubmit={event => { event.preventDefault(); loginMutation.mutate({ email, password }); }} className="mt-5 space-y-3"><label className="block text-xs font-bold text-[#466381]">Email address<input aria-label="Email address" value={email} onChange={event => setEmail(event.target.value)} type="email" required autoComplete="email" className="mt-1.5 w-full rounded-xl border border-[#d8e4f0] bg-[#f8fbff] px-3 py-2.5 text-xs font-semibold text-[#365575] outline-none focus:border-[#5f9cf7] focus:ring-4 focus:ring-blue-100" /></label><label className="block text-xs font-bold text-[#466381]">Password<input aria-label="Password" value={password} onChange={event => setPassword(event.target.value)} type="password" required autoComplete="current-password" className="mt-1.5 w-full rounded-xl border border-[#d8e4f0] bg-[#f8fbff] px-3 py-2.5 text-xs font-semibold text-[#365575] outline-none focus:border-[#5f9cf7] focus:ring-4 focus:ring-blue-100" /></label><button disabled={loginMutation.isPending} className="w-full rounded-xl bg-[#0b57d0] px-4 py-3 text-xs font-bold text-white shadow-[0_8px_16px_rgba(11,87,208,.16)] disabled:opacity-60">{loginMutation.isPending ? "Signing in…" : "Enter Workforce Hub"} <ArrowRight className="ml-1 inline" size={14} /></button><button type="button" onClick={() => { setMessage(""); setMode("forgot"); }} className="w-full text-center text-xs font-bold text-[#0b57d0]">Forgot password?</button></form></>}{mode === "forgot" && <form onSubmit={event => { event.preventDefault(); requestResetMutation.mutate({ email }); }} className="mt-5 space-y-3"><label className="block text-xs font-bold text-[#466381]">Email address<input aria-label="Reset email" value={email} onChange={event => setEmail(event.target.value)} type="email" required autoComplete="email" className="mt-1.5 w-full rounded-xl border border-[#d8e4f0] bg-[#f8fbff] px-3 py-2.5 text-xs font-semibold text-[#365575] outline-none focus:border-[#5f9cf7] focus:ring-4 focus:ring-blue-100" /></label><button disabled={requestResetMutation.isPending} className="w-full rounded-xl bg-[#0b57d0] px-4 py-3 text-xs font-bold text-white disabled:opacity-60">{requestResetMutation.isPending ? "Preparing code…" : "Generate reset code"}</button><button type="button" onClick={() => setMode("login")} className="w-full text-center text-xs font-bold text-[#0b57d0]">Return to sign in</button></form>}{mode === "reset" && <form onSubmit={event => { event.preventDefault(); if (newPassword !== confirmPassword) { setMessage("Passwords do not match."); return; } resetMutation.mutate({ token: resetToken, password: newPassword }); }} className="mt-5 space-y-3"><label className="block text-xs font-bold text-[#466381]">Reset code<input aria-label="Reset code" value={resetToken} onChange={event => setResetToken(event.target.value)} required className="mt-1.5 w-full rounded-xl border border-[#d8e4f0] bg-[#f8fbff] px-3 py-2.5 text-xs font-semibold text-[#365575] outline-none" /></label><label className="block text-xs font-bold text-[#466381]">New password<input aria-label="New password" value={newPassword} onChange={event => setNewPassword(event.target.value)} type="password" minLength={12} required autoComplete="new-password" className="mt-1.5 w-full rounded-xl border border-[#d8e4f0] bg-[#f8fbff] px-3 py-2.5 text-xs font-semibold text-[#365575] outline-none" /></label><label className="block text-xs font-bold text-[#466381]">Confirm password<input aria-label="Confirm password" value={confirmPassword} onChange={event => setConfirmPassword(event.target.value)} type="password" minLength={12} required autoComplete="new-password" className="mt-1.5 w-full rounded-xl border border-[#d8e4f0] bg-[#f8fbff] px-3 py-2.5 text-xs font-semibold text-[#365575] outline-none" /></label><button disabled={resetMutation.isPending} className="w-full rounded-xl bg-[#0b57d0] px-4 py-3 text-xs font-bold text-white disabled:opacity-60">{resetMutation.isPending ? "Resetting…" : "Save new password"}</button></form>}{(loginMutation.error || requestResetMutation.error || resetMutation.error) && <p className="mt-4 rounded-xl bg-rose-50 p-3 text-xs font-semibold text-rose-700">{loginMutation.error?.message || requestResetMutation.error?.message || resetMutation.error?.message}</p>}{message && <p className="mt-4 rounded-xl bg-[#effaf7] p-3 text-xs font-semibold text-[#267669]">{message}</p>}<p className="mt-6 text-center text-[10px] leading-4 text-[#8193a9]">If you need access, contact your Workforce Hub administrator.</p></div></div></section></div></div>;
}
...
function Workspace({ exitWorkspace, requestedPage = "Overview" }: { exitWorkspace: () => void; requestedPage?: string }) {
  ...
  const resumeParserMutation = trpc.recruiting.parseResume.useMutation({ onSuccess: data => { setResumeProfile(data); setResumePrepared(false); } });
